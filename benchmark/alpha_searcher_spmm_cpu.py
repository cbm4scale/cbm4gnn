...
def correctness(edge_index):
    ...
    for alpha in alphas:
        a_csr = torch.sparse_coo_tensor(edge_index, values, mat.shape).to_sparse_csr()
        a_cbm = cbm4mm(edge_index.to(torch.int32), values, alpha=alpha)
        # a_cbm.check_format(edge_index)
        ...
        for size in sizes:
            ...
            a_csr_x = torch.empty(dim_size, size, dtype=x.dtype)
            mkl_matmul(a_csr, x, a_csr_x)
...
def timing(edge_index, name):
    ...
    for size in sizes:
        ...
        for alpha_i in alphas:
            c = cbm4mm(edge_index.to(torch.int32), values, alpha=alpha_i)
            ...
            start_time = time.time()
            for _ in range(iters):
                # no need to have this line and it hurts performance
                #y3 = torch.empty(dim_size, size, dtype=x.dtype)
                c.matmul(x, y3)
