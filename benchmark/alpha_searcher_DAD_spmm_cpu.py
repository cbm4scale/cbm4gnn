...
def correctness(edge_index):
    ...
    normalized_values = normalize_edge_index(edge_index.to(torch.int64))
    #check_edge_index(edge_index)

    dim_size = rowptr.size(0) - 1

    a_csr = torch.sparse_coo_tensor(edge_index, normalized_values, mat.shape).to_sparse_csr()
    
    for alpha in alphas:
        
        a_cbm = cbm4gcn(edge_index.to(torch.int32), values, alpha=alpha)
        ...
        for size in sizes:
            ...
            a_mkl_x = torch.empty(dim_size, size, dtype=x.dtype)
            mkl_matmul(a_csr, x, a_mkl_x)
            ...
            torch.testing.assert_close(a_csr_x, a_mkl_x, atol=1e-4, rtol=1e-4)
...
def timing(edge_index, name):
    ...
    a = torch.sparse_coo_tensor(edge_index, normalized_values, mat.shape).to_sparse_csr()
    ...
    for size in sizes:
        ...
        for alpha_i in alphas:
            c = cbm4gcn(edge_index.to(torch.int32), values, alpha=alpha_i)
