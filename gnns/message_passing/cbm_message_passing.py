    ...
    def forward(self, edge_index, size=None, **kwargs):
        ...
        if self.cached:
            if self.cached_a_t is None:
                ...
                self.cached_a_t = cbm_matrix(edge_index_t, ones(edge_index.size(1), dtype=x.dtype), normalized=self.normalize, alpha=self.alpha)
            ...
            edge_index_t = stack([edge_index[1], edge_index[0]], dim=0).to(int32)
            a_t = cbm_matrix(edge_index_t, ones(edge_index.size(1), dtype=x.dtype), normalized=self.normalize, alpha=self.alpha)
            out = empty(size=(edge_index.max().item() + 1, x.size(1),), dtype=x.dtype, device=x.device)

        a_t.seq_mkl_csr_fused_spmm_update(x, out) if self.normalize else a_t.seq_mkl_csr_spmm_update(x, out)
        return out
...
class CBMParallelMKLCSRSparseMatrixMessagePassing(CBMSequentialMKLCSRSparseMatrixMessagePassing):
    def forward(self, edge_index, size=None, **kwargs):
        ...
        a_t.omp_mkl_csr_fused_spmm_update(x, out) if self.normalize else a_t.omp_mkl_csr_spmm_update(x, out)
        return out
...
class CBMSequentialTorchCSRSparseMatrixMessagePassing(MessagePassing):
    ...
    def forward(self, edge_index, size=None, **kwargs):
        ...
        out = a_t.seq_torch_csr_fused_matmul(x) if self.normalize else a_t.seq_torch_csr_matmul(x)
        return out
...
class CBMParallelTorchCSRSparseMatrixMessagePassing(CBMSequentialTorchCSRSparseMatrixMessagePassing):
    def forward(self, edge_index, size=None, **kwargs):
        ...
        out = a_t.omp_torch_csr_fused_matmul(x) if self.normalize else a_t.omp_torch_csr_matmul(x)
        return out